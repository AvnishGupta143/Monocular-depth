...
class MonodepthModel(object):
    ...
    def build_vgg(self):
        ...
        # Added the Semantic Decoder
        # TODO: check removed "disp" parts not needed
        with tf.variable_scope('semantic_decoder'):
            upconv7_semantic = upconv(conv7, 512, 3, 2)  # H/64
            concat7_semantic = tf.concat([upconv7_semantic, skip6], 3)
            iconv7_semantic = conv(concat7_semantic, 512, 3, 1)

            upconv6_semantic = upconv(iconv7_semantic, 512, 3, 2)  # H/32
            concat6_semantic = tf.concat([upconv6_semantic, skip5], 3)
            iconv6_semantic = conv(concat6_semantic, 512, 3, 1)

            upconv5_semantic = upconv(iconv6_semantic, 256, 3, 2)  # H/16
            concat5_semantic = tf.concat([upconv5_semantic, skip4], 3)
            iconv5_semantic = conv(concat5_semantic, 256, 3, 1)

            upconv4_semantic = upconv(iconv5_semantic, 128, 3, 2)  # H/8
            concat4_semantic = tf.concat([upconv4_semantic, skip3], 3)
            iconv4_semantic = conv(concat4_semantic, 128, 3, 1)
            # self.disp4_semantic = self.get_disp(iconv4_semantic)  # Removed disp section from semantic
            # udisp4_semantic  = self.upsample_nn(self.disp4_semantic, 2)  # Removed disp section from semantic

            upconv3_semantic = upconv(iconv4_semantic, 64, 3, 2)  # H/4
            # concat3_semantic = tf.concat([upconv3_semantic, skip2, udisp4_semantic], 3)  # Removed disp section from semantic
            concat3_semantic = tf.concat([upconv3_semantic, skip2], 3)
            iconv3_semantic = conv(concat3_semantic, 64, 3, 1)
    ...
    def build_resnet50(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            # SEMANTIC DECODING
            # TODO: replace the "disp" sections of the semantic decoder
            with tf.variable_scope('semantic_decoder'):
                upconv6_semantic = upconv(conv5, 512, 3, 2)  # H/32
                concat6_semantic = tf.concat([upconv6_semantic, skip5], 3)
                iconv6_semantic = conv(concat6_semantic, 512, 3, 1)
                ...
                upconv3_semantic = upconv(iconv4_semantic, 64, 3, 2)  # H/4
                # concat3_semantic = tf.concat([upconv3_semantic, skip2, udisp4_semantic], 3)
                concat3_semantic = tf.concat([upconv3_semantic, skip2], 3)  # Removed disp section from semantic
                iconv3_semantic = conv(concat3_semantic, 64, 3, 1)
                # self.disp3_semantic = self.get_disp(iconv3_semantic)  # Removed disp section from semantic
                # udisp3_semantic = self.upsample_nn(self.disp3_semantic, 2)  # Removed disp section from semantic
