...
class MonodepthModel(object):
    ...
    def build_vgg(self):
        ...
        #     unet_model.load_weights('UNetSegmentationAdamDiceLoss.h5')
            ...
            # unet_model = keras.models.load_model('cityscapes/unet_cityscapes.h5',custom_objects={'Functional':tf.keras.models.Model})
            # unet_model = self.get_model()
            # unet_model.load_weights('UNetSegmentationAdamDiceLoss.h5')
        #     unet_model = self.get_model()
        #     unet_model.load_weights('UNetSegmentationAdamDiceLoss.h5')
        #     # unet_model = keras.models.load_model('UNetSegmentationAdamDiceLoss.h5')
            # for layer in unet_model.layers:
            #     layer.trainable = False
        ...
        #     # 128 x 128
        #     # skipSemantic2 = tf.keras.models.Model(inputs=unet_model.input, outputs=unet_model.get_layer('batch_normalization_18').output)
        #     # # res = last_layer.predict(image_batch)

        #     # # 64 X 64
        #     # skipSemantic3 = tf.keras.models.Model(inputs=unet_model.input, outputs=unet_model.get_layer('batch_normalization_15').output)

        #     # # 32 X 32
        #     # skipSemantic4 = tf.keras.models.Model(inputs=unet_model.input, outputs=unet_model.get_layer('batch_normalization_12').output)
    ...
    def build_summaries(self):
        ...
        # for i, d in enumerate(['/gpu:8','/gpu:9']):
